...
import { Code, Copy, Import, Send, Plus, Trash, ExternalLink, Check, AlertCircle, X, Image, Save, FolderOpen, Share2 } from 'lucide-react';
...
function EmbedBuilder() {
  ...
  // Load saved embeds from localStorage on mount
  useEffect(() => {
    const saved = localStorage.getItem('savedEmbeds');
    if (saved) {
      setSavedEmbeds(JSON.parse(saved));
    }
  }, []);

  // Check URL hash for shared embed data on mount
  useEffect(() => {
    const hash = location.hash.slice(1); // Remove the # symbol
    if (hash) {
      try {
        const decodedData = JSON.parse(decodeURIComponent(hash));
        setEmbed(decodedData);
        showNotification(
          <Check className="h-6 w-6" />,
          "Embed loaded",
          "Successfully loaded shared embed"
        );
        // Clear the hash to prevent reloading on refresh
        navigate(location.pathname, { replace: true });
      } catch (error) {
        console.error('Failed to parse shared embed data:', error);
        showNotification(
          <AlertCircle className="h-6 w-6" />,
          "Error",
          "Failed to load shared embed",
          'error'
        );
      }
    }
  ...
  const handleShare = () => {
    ...
    const encodedData = encodeURIComponent(embedData);
  ...
  const handleSaveEmbed = () => {
    if (!embedName.trim()) {
      showNotification(
        <AlertCircle className="h-6 w-6" />,
        "Error",
        "Please enter a name for your embed",
        'error'
      );
      return;
    }
    ...
    const existingIndex = savedEmbeds.findIndex(e => e.name === embedName);
    let updatedEmbeds: SavedEmbed[];

    if (existingIndex !== -1) {
      // Update existing embed
      updatedEmbeds = [...savedEmbeds];
      updatedEmbeds[existingIndex] = newSavedEmbed;
    } else {
      // Add new embed
      updatedEmbeds = [...savedEmbeds, newSavedEmbed];
    }

    setSavedEmbeds(updatedEmbeds);
    localStorage.setItem('savedEmbeds', JSON.stringify(updatedEmbeds));
    
    showNotification(
      <Check className="h-6 w-6" />,
      "Embed saved",
      `Embed "${embedName}" has been saved successfully`
    );
  ...
  const handleLoadEmbed = (savedEmbed: SavedEmbed) => {
    setEmbed(savedEmbed.data);
    showNotification(
      <Check className="h-6 w-6" />,
      "Embed loaded",
      `Embed "${savedEmbed.name}" has been loaded successfully`
    );
    setShowLoadModal(false);
  };

  const handleDeleteEmbed = (name: string) => {
    const updatedEmbeds = savedEmbeds.filter(e => e.name !== name);
    setSavedEmbeds(updatedEmbeds);
    localStorage.setItem('savedEmbeds', JSON.stringify(updatedEmbeds));
  ...
  const formatDiscordText = (text: string) => {
    if (!text) return '';
    
    // First replace variables, then format markdown
    const textWithVars = replaceVariables(text);
  ...
  const handleSendWebhook = async () => {
    try {
      ...
      showNotification(
        ...
        "Webhook sent",
        "The embed has been sent to the webhook successfully."
      );
      ...
      showNotification(
        <AlertCircle className="h-6 w-6" />,
        "Webhook failed",
        "Failed to send the embed to the webhook. Please check the URL and try again.",
        'error'
      );
    }
  };
    ...
    <div className="max-w-[90rem] mx-auto px-4 py-8">
      <div className="text-center mb-16">
        <div className="w-16 h-16 bg-theme/10 rounded-xl flex items-center justify-center mx-auto mb-6">
          <Code className="w-8 h-8 text-theme" />
        </div>
        <h1 className="text-4xl font-bold mb-4">Embed Builder</h1>
        <p className="text-xl text-gray-400 max-w-2xl mx-auto">
          Create beautiful Discord embeds with our visual editor
        </p>
      </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-20">
          <div className="feature-card rounded-lg p-6 border border-dark-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <div className="grid grid-cols-1 sm:grid-cols-6 gap-2 flex-1">
                <button
              ...
              <div className="border-l-4 relative" style={{ borderColor: embed.color }}>
                ...
                {previewThumbnailUrl && !thumbnailError && (
                  <div className="absolute -top-2 -right-2 w-20 h-20">
                    <img 
                      src={replaceVariables(previewThumbnailUrl)}
                      alt="" 
                ...
                <div className="pl-4">
                  {embed.author && (
                    <div className="flex items-center gap-2 mb-2">
                      {embed.author.icon && (
                        <img src={replaceVariables(embed.author.icon)} alt="" className="w-6 h-6 rounded-full" />
                      )}
                      {embed.author.url ? (
                        <a href={embed.author.url} className="text-blue-400 hover:underline break-words" target="_blank" rel="noopener noreferrer">
                  ...
                  {previewImageUrl && !imageError && (
                    <div className="max-w-full mb-4">
                      <img 
                        src={replaceVariables(previewImageUrl)}
                        alt="" 
                  ...
                  {embed.footer && (
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                      {embed.footer.icon && (
                        <img src={replaceVariables(embed.footer.icon)} alt="" className="w-5 h-5 rounded-full" />
                      )}
      ...
      >
        <div className="space-y-4">
          {savedEmbeds.length === 0 ? (
            <p className="text-center text-gray-400">No saved embeds found.</p>
          ) : (
            <div className="space-y-2">
              {savedEmbeds.map((savedEmbed) => (
                <div
                  key={savedEmbed.name}
                  className="flex items-center justify-between p-3 bg-dark-2 rounded-lg"
                >
                  <div>
                    <h4 className="font-medium">{savedEmbed.name}</h4>
                    <p className="text-sm text-gray-400">
                      {new Date(savedEmbed.savedAt).toLocaleString()}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleLoadEmbed(savedEmbed)}
                      className="p-2 bg-theme/20 hover:bg-theme/30 text-theme rounded transition-colors"
                    >
                      <FolderOpen className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => handleDeleteEmbed(savedEmbed.name)}
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-400">Share this link to let others load your embed:</p>
          <div className="flex gap-2">
            <input
              type="text"
              readOnly
            ...
            <button
              ...
              className="px-4 py-2 bg-theme hover:bg-theme/90 text-white rounded-lg transition-colors flex items-center gap-2"
            >
              <Copy className="w-4 h-4" />
              Copy
            </button>
